...
function test_sim(net::NetworkParameters, scenario_number::Int64;
    ...
    Threads.@threads for index in eachindex(ρ_star_values)
        ...
        if verbose
            # clear line and move cursor to column 1
            print("\e[2K")
            print("\e[1G")
            if multithreaded
                print("Running ρ* = $ρ_star in thread $(Threads.threadid())")
            else
                print("Running ρ* = $ρ_star")
            end
        end
        ...
        # println("$theoretical")
        # println(minimum(simulated))

        # save lengths to array
        sim_total_mean_lengths[index] = [theoretical, simulated...]
    ...
    sim_total_mean_lengths = [arr[k] for k in 1:dim+1, arr in sim_total_mean_lengths]'
    ...
    return plot(
        ...
        labels=["theoretical" hcat("max_time: " .* string.(max_times)...)],
